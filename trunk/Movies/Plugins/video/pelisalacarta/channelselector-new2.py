...
def getChannels():
    ...
    pfolder = join( config.get_runtime_path() , "pelisalacarta", "channels")
    ...
    for f in listdir(pfolder):
        if (isfile(join(pfolder, f)) and f.endswith(".py")) and not f.startswith("_"):
            ...
            if channel and working and (not adult or config.get_setting("enableadultmode")):
                logger.debug("append title=" + title[0] + ", channel=" + channel[0]+", language=" + language[0]+", category=" + category[0]+", type=" + type[0]+", creationdate=" + str(creationdate)+", working=" + str(working))
