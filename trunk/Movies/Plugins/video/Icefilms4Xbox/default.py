...
#Icefilms.info v1.0.10 - anarchintosh / daledude / westcoast13 2011-07-02 b
...
def resolve_180upload(url):
    ...
    try:
        ...
        dialog.create('Resolving', 'Kozz is the man :-) Resolving 180Upload Link...')
        dialog.update(0)
               ...
               for sourcescrape1,sourcescrape2 in multi_part_source:
                    ...
                    for id, partnum in pair:
                        ...
                        if ismega is not None:
                              ...
                              Add_Multi_Parts(fullname,url,u180pic)
                        elif isspeedy:
                              fullname=sourcestring+' | SS | '+partname
                              Add_Multi_Parts(fullname,url,speedypic)
                        elif isvidhog:
                              fullname=sourcestring+' | VH | '+partname
                              Add_Multi_Parts(fullname,url,vihogpic)
                        elif isuploadorb:
                              fullname=sourcestring+' | UO | '+partname
                              Add_Multi_Parts(fullname,url,uploadorbpic)
                        elif issharebees:
          ...
          # if source does not have multiple parts...
               ...
               for id in source5:
                    ...
                    if ismega is not None:
                         ...
                         # print 'Source #'+sourcenumber+' is hosted by 180upload'
                         fullname=sourcestring+' | 180 | Full'
                         addExecute(fullname,url,200,u180pic)
                    elif isspeedy is not None:
                         # print 'Source #'+sourcenumber+' is hosted by speedy' 
                         fullname=sourcestring+' | SS  | Full'
                         addExecute(fullname,url,200,speedypic)
                    elif isvidhog is not None:
                         #print 'Source #'+sourcenumber+' is hosted by vidhog' 
                         fullname=sourcestring+' | VH  | Full'
                         addExecute(fullname,url,200,vihogpic)
                    elif isuploadorb is not None:
                         #print 'Source #'+sourcenumber+' is hosted by 2shared' 
                         fullname=sourcestring+' | UO  | Full'
                         addExecute(fullname,url,200,uploadorbpic)     
