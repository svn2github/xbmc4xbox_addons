...
register.RegisterChannel('chn_123video')
...
class Channel(chn_class.Channel):
    ...
    def InitialiseVariables(self):
        ...
        self.videoItemRegex = '<a onFocus="this.blur\(\);" href="/playvideos.asp\?MovieID=(\d+)[^"]+" title="([^"]+)" [^>]+><img src="([^"]+)" width="\d+" /></a>[^:]+:[^:]+:[^:]+[^"]+"[^"]+"> *(\d+)-(\d+)-(\d+)'
...
#    <a onfocus="this.blur();" href="/video.asp?So=3" title="Meest besproken video's" onmouseover="fOver(this);return true;">Meest besproken video's</a>
    ...
    def UpdateVideoItem(self, item):
        ...
        firstPartLength = len(item.url) - 3
        firstPart = item.url[0:firstPartLength]
        mediaurl = "http://%s/%s/%s.flv" % (ip, firstPart, item.url)
