...
class cHoster(iHoster):
  ...
  def __getMediaLinkForGuest(self):
    ...
    sPostValue = ""
    sPostButtonName = ""
    sPattern = '<form method="post".*?<input type="hidden" value="([^"]+)" name="([^"]+)".*?<input name="([^"]+)" type="submit"'#'<form.*?<input type="hidden" value="([^"]+)" name="([^"]+)".*?<input name="([^"]+)" type="submit"'
    ...
    oRequest.addParameters(sPostButtonName, "Continue as Free User")
    sHtmlContent = oRequest.request()
    ...
    # if not aResult[0]:
      ...
      # return [False, ""]
    oRequest = cRequestHandler("http://www.sockshare.com" + aResult[1][0])
    sHtmlContent = oRequest.request()
