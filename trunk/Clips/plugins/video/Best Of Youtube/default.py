...
def index():
        addDir("Newest 100","NEW",1,"")
        addDir("BestOf: Week","http://www.bestofyoutube.com/index.php?show=week",3,"")
        addDir("BestOf: Month","http://www.bestofyoutube.com/index.php?show=month",3,"")
        addDir("BestOf: Year","http://www.bestofyoutube.com/index.php?show=year",3,"")
        addDir("BestOf: AllTime","http://www.bestofyoutube.com/index.php?show=alltime",3,"")
...
def playVideo(url):
        if url.find("http://")==0:
          content = getUrl(url)
          #match=re.compile('<div class=\'title\'>(.+?)</div>', re.DOTALL).findall(content)
          #title=match[0]
          match=re.compile('<param name="movie" value="http://www.youtube.com/v/(.+?)&amp;', re.DOTALL).findall(content)
          youtubeID=match[0]
        else:
          youtubeID=url
        #xbmc.executebuiltin('XBMC.Notification(Info, '+title+';'+youtubeID+', 5000)')
