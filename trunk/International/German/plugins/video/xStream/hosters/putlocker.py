...
class cHoster(iHoster):
  ...
  FILE_INFO_URL = "http://www.putlocker.com/get_file.php?stream="
  ...
  def __getMediaLinkForGuest(self):
    ...
    sPostButtonName = ""
    sPostButtonValue = ""
    sPattern = '<form.*?<input type="hidden" value="([^"]+)" name="([^"]+)".*?<input name="([^"]+)" type="submit" value="([^"]+)"'
    #sPattern = 'type="hidden" value="(.+?)" name="hash"'
    oParser = cParser()
    ...
    if (aResult[0] == True):
        for aEntry in aResult[1]:
            ...
            sPostButtonValue = aEntry[3]
            
    log("Form data: %s %s %s %s" % (sPostName, sPostValue, sPostButtonName, sPostButtonValue), LOGDEBUG)
    ...
    oRequest.addParameters(sPostButtonName, sPostButtonValue)
    sHtmlContent = oRequest.request()
    ...
    aResult = oParser.parse(sHtmlContent, sPattern)
    if not aResult[0]:
       log("No streamid found.", LOGERROR)
       return [False, ""]
	
    # Now we can try to get the id.
    log("Prepared config URL: %s" % cHoster.FILE_INFO_URL + str(aResult[1][0]))

    oRequest = cRequestHandler(cHoster.FILE_INFO_URL + str(aResult[1][0]))
