  ...
  if (aResult[0]):
    for aEntry in aResult[1]:
            ...
            if (oHoster != False):
                ...
                #oOutputParameterHandler.addParameter('hosterName', oHoster.get_host())
...
def getHosterUrlandPlay():
  ...
  if (aResult[0]):
    ...
    if (len(aMovieParts) > 1):
        ...
        oHoster = cHosterHandler().getHoster2(sPartUrl)
        cHosterGui().showHosterMenuDirect(oGui, oHoster, sPartUrl)
  oGui.setEndOfDirectory()
...
def multiPartMovie():
    ...
    sTitle = oInputParameterHandler.getValue('sTitle')    
    cHosterGui().showHosterMenuDirect(oGui, oHoster, sPartUrl, sTitle)
    oGui.setEndOfDirectory()
