...
def __getSecurityCookieValue():
    ...
    sPattern = "var hash=\[(.*?)\]"#"<HTML><HEAD><SCRIPT language=\"javascript\" src=\"([^\"]+)\"></SCRIPT></HEAD>.*?<BODY onload=\"scf\('(.*?)','/'\)\"></BODY></HTML>"
    #log(sPattern)
    oParser = cParser()
    aResult = oParser.parse(sHtmlContent, sPattern)
    #log(URL_MAIN + " : " + sHtmlContent)
    if aResult[0] == False:
        print "Can't find script file for cookie"
        return False
    for aEntry in aResult[1][0].split(","):
		sScriptFile = URL_MAIN + str(aEntry).replace("'","").strip()#str(aResult[1][0][0])
		sScriptFile.replace(" ","")
