...
URL_FILTER_SETTINGS = 'http://iload.to/de/settings/filter/'
...
def showRelease():
    ...
    sPattern = '<input type="hidden" name="i_am_old_enough" value="(.*?)">'
    oParser = cParser()
    aResult = oParser.parse(sHtmlContent, sPattern)
    ...
    if (aResult[0] == True):
		...
		oRequest.addParameters('i_am_old_enough', aResult[1][0])
