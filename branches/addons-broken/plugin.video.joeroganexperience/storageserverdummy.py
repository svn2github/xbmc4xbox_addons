...
if hasattr(sys.modules["__main__"], "settings"):
    ...
    settings = False
__Addon__ = xbmcaddon.Addon(id='plugin.video.icefilms')
...
class StorageServer:
    def __init__(self, table=False,cacheTime=0):
        self.table = table
        if settings:
            temporary_path = xbmc.translatePath(_Addon_.getAddonInfo("profile"))
...
#profile_path = self.Addon.getAddonInfo(id="profile")
        #local = xbmc.translatePath(self.Addon.getAddonInfo(id="profile"))
        local = xbmc.translatePath(__Addon__.getAddonInfo(id='profile'))
